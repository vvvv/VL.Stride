...
//            contentIndexMap = !string.IsNullOrEmpty(indexName) ? global::Stride.Core.Serialization.Contents.ContentIndexMap.Load(vfsRootUrl + VirtualFileSystem.DirectorySeparatorChar + indexName, isReadOnly)
//                                                             : global::Stride.Core.Serialization.Contents.ContentIndexMap.CreateInMemory();
...
//            Stream stream = new MemoryStream();// virtualFileProvider.OpenStream(tmpFileName, VirtualFileMode.Create, VirtualFileAccess.Write);
//            return new DigestStream(stream) { Disposed = x => MoveToDatabase(stream, x.CurrentHash), TemporaryName = tmpFileName };
//        }

//        private void MoveToDatabase(MemoryStream stream, ObjectId objId, bool forceWrite = false)
//        {
//            stream.GetBuffer();
//            stream.Length;
